import React, { useState, useEffect, useRef, BaseSyntheticEvent, FormEvent } from 'react'
...
interface ValidationType {
  ...
  TextLengthBelow30: string,
  TextLengthBelow50: string,
  TextLengthBelow100: string,
  TextLengthBelow200: string,
  TextLengthBelow300: string,
  TextLengthBelow400: string,
  Number: string,
  Price: string,
  validationType: string
}

interface EditEntryType {
  Text: string,
  TextList: String,
  DoubleTextList: string,
  TextArea: string,
  File: string,
  Address: string,
  Photo: string,
...
export const EditEntryType: EditEntryType = {
  Text: 'Text',
  TextList: 'TextList',
...
export const ValidationType: ValidationType = {
  Email: 'Email',
...
function validateValue(value: any, attributeName: String, { validationType }: ValidationType) {
...
export function EditForm(props: EditFormProps) {
  ...
  const [characterCounts, setCharacterCounts] = useState<CharacterCount>({});

  const [uploadPhotoMap, setUploadPhotoMap] = useState<{ [key: string]: string }>({})
  ...
  useEffect(() => {
    const currListFieldSize = props.editEntries.map((entry: EditEntry) => {
      ...
      <form className="space-y-8 divide-y divide-gray-200"
        onSubmit={(event: React.FormEvent<HTMLFormElement>) => {
          ...
          for (const element of form.elements) {
            const target = element as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | any
            ...
            if (updateTargets.has(target.name)) {
              ...
              const entity_sub_field: any = target.name.split("_").slice(1).join("_");
          ...
          for (const editEntry of props.editEntries) {
            ...
            if (max_idx_to_take > 0) {
              entity[editEntry.attribute] = Object.fromEntries(Object.entries(entity[editEntry.attribute] as EntityValue).filter(([k, v]) => parseInt(k, 10) < max_idx_to_take && v["0"] && v["1"]));
            }
            ...
            if (editEntry.type === EditEntryType.Showcase) {
              ...
              const numPhotos = showcase.image_urls.filter((image_url: any) => image_url.state !== StoredFileState.Deleted).length;
                  ...
                  if (!editEntry.type || editEntry.type == EditEntryType.Text) {
                    ...
                    return (
                      <FileUpload
                        ...
                        uponFileChange={(files: [string]) => entity[editEntry.attribute] = files}
                        isMultiple={true}
                      />
                    );
